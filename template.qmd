...
```{julia}
function get_profit(capacity, m, b)
    price = div(capacity - b, m) # Floor Div for dollars
    return (price * capacity)-(30000 * capacity)
end
```

```{julia}
function optimize(maxSpots, increment, start_spots, m)
    b = maxSpots
    levels_to_add = div(maxSpots - start_spots, increment)
    optLevels = 0 
    optProfit = get_profit(start_spots, m, b)
    curr_levels = 0
    while curr_levels < levels_to_add
        curr_capacity = start_spots + curr_levels * increment
        profit = get_profit(curr_capacity, m, b)
        if profit > optProfit
            optProfit = profit
...
opt_levels_to_add = optimize(maxSpots, increment, start_spots, m)

function simulation(timesteps, start_spots, increment, demand_growth_rate, init_b, m, inflation_rate)
    ...
    while timestep < timesteps
        levels_to_add = optimize(b, increment, curr_spots, m)
        if levels_to_add == 0
            # println("Whoops")
        end
        ...
        curr_profit = get_profit(curr_spots, m, b) / 1000
...
```{julia}
y_levels = simulation(25, 0, 200, rand(Normal(0.0045, 0.001)), 1500 , -0.046, 0.04)
println(y_levels)
plot(y_levels, label="Optimate Levels at Time t", xlabel="Year", ylabel="Number of Levels",
title="Parking Garage Simulation", lw=1, gridlinewidth=1, xticks=0:1:25, yticks=0:1:100)
